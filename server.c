...
void intHandler(int dummy) {
    // close(new_sock);
	printf("[info] Received  SIGINT, closing socket\n");
	close(server_fd);
	exit(0);
}
...
void * worker(void *args){
	...
	while(1)
	{
		...
		memset(fname, 0, sizeof(fname));//flush
		printf("\t[info] waiting for msg from tid #%d\n",tid);
		read_return = recv( new_sock ,(void *) fname, sizeof(fname), 0 );
		if(read_return<0){
			perror("\t[error] in reading\n");
			exit(1);
		}
		else if(!read_return){	
			printf("\t[info] exiting, from tid #%d\n",tid);
			break;
		}
		
		printf("\t[info]C, tid #%d requested: ", tid);
		printf("%s\n",fname);
		//open file
		FILE *fp = fopen(fname,"rb"); //IMP DOUBT 
		if(fp){//exists
			stat(fname, &st);
			...
			printf("[info] file perms: %o\n",statchmod);
			memset(buffer, 0, sizeof(buffer));//flush
			sprintf(buffer,"%o",statchmod);
			if (send(new_sock, buffer, sizeof(buffer), 0) < 0){
				perror("[error] in send");
				exit(1);
			}
			...
			printf("[info] %ld chunks",req_loops);
			fclose(fp);
		}
...
int main(int argc, char const *argv[])
{
    ...
    {
        printf("[error] addess converesion error\n");
        exit (1);
    }
	...
	while(1)
	{// NEW CLIENT
		printf("[info] waiting for new client, current count: %d\n",client_count);
		...
		printf("[info] rcvd_client, client fd: %d\n", new_sock);

		//lock 
		sem_wait(&sem);
		printf("[info] checking if lim of %d reached, current count is %d\n",MAXCONN,client_count);
		if(client_count<MAXCONN){
			...
			close(new_sock);
			printf("[warining] REJECTED, max clients\n");
		}
		//unlock
		sem_post(&sem);
	}
