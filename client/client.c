...
int main(int argc, char const *argv[])
{
	...
	while(1)
	{
		...
		scanf("%[^\n]%*c", fname);
        printf("Scanned: %s\n",fname);
		if ( (strlen(fname)==strlen("quit")) && (strncmp(fname,"quit",strlen("quit"))==0))
		{
			printf("# exiting\n");
			break;
		}
		if (send(sock, fname, strlen(fname), 0) < 0){
			perror("# error in send");
			exit(1);
		}
        FILE *fp = fopen(fname,"wb"); //IMP DOUBT
        if(!fp){
        ...
        printf("File perms: %s\n", buffer);
        sprintf(cmd, "chmod %s %s",buffer, fname);
        system(cmd);
